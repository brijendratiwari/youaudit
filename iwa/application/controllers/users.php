...
//                            redirect($strReferral, 'refresh');
                    ...
                    if ($usercheck['result'][0]->level_id == 5) {
                        $this->session->set_userdata('booUserLogin', FALSE);
                        $arrPageData['arrErrorMessages'][] = "You do not have access to this system";
                    }
                    $this->session->set_userdata('booUserLogin', FALSE);
...
// load views
    ...
    public function logout() {
        $this->session->unset_userdata(array('booUserLogin', 'objSystemUser','theme_design'));
                ...
                if (file_exists("./excel_file/" . $_FILES['file']['name'])) {
                    ...
                    for ($i = 1; $i <= count($sheetData); $i++) {
                        if ($i == 1) {
                            ...
                            // check user avaibility in database
                            $boolvalue = $this->users_model->checkUserName($sheetData[$i]['B']);
                            if ($boolvalue) {
                                
                            } else {
                                switch ($sheetData[$i]['A']) {
                                    case 'user':
                                        $level_id = 1;
                                        break;
                                    case 'manager':
                                ...
                                $arrInput = array(
                                    ...
                                    'lastname' => $sheetData[$i]['E'],
                                    'level_id' => $level_id,
                                    'photo_id' => 1,
                                    'username' => $sheetData[$i]['B'],
                                    'password' => md5($sheetData[$i]['C']),
                                    'nickname' => $sheetData[$i]['D'],
                                );
...
// Log it first
                ...
                unlink("./excel_file/" . $_FILES['file']['name']);
                redirect('users/exportExcelData/', 'refresh');
            } else {
    ...
    public function downloadExcelFomatFile() {
//load our new PHPExcel library
        $this->load->library('excel');
//activate worksheet number 1
        $this->excel->setActiveSheetIndex(0);
//name the worksheet
        $this->excel->getActiveSheet()->setTitle('User Add Sheet');
//set cell A1 content with some text
        $this->excel->getActiveSheet()->setCellValue('A1', 'level');
        $this->excel->getActiveSheet()->setCellValue('B1', 'username');
        $this->excel->getActiveSheet()->setCellValue('C1', 'password');
        $this->excel->getActiveSheet()->setCellValue('D1', 'firstname');
        $this->excel->getActiveSheet()->setCellValue('E1', 'lastname');
...
//$this->excel->getActiveSheet()->getStyle('A1')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
        ...
        $filename = 'demoExcel.xlsx'; //save our workbook as this file name
...
//force user to download the Excel file without writing it to server's HD
    ...
    public function exportExcelData() {
        ...
        $arrPageData = array();
        $arrPageData['arrPageParameters']['strSection'] = get_class();
        $arrPageData['arrPageParameters']['strPage'] = "Export Excel Data";
        $arrPageData['arrSessionData'] = $this->session->userdata;
        $this->session->set_userdata('booCourier', false);
        $this->session->set_userdata('arrCourier', array());
        $arrPageData['arrErrorMessages'] = array();
        ...
        $this->load->view('users/exportexcel', $arrPageData);
