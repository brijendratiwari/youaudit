...
//                $('#tokenfield-typeahead').tokenfield('createToken', {value: arr[0], label: arr[1]});
    ...
    function beforeTestAdd() {
        ...
        if (comName.trim() == '' || comCat.trim() == '' || comFre.trim() == '') {
            ...
            $('select#mandatory[value=""]').effect('shake', {times: 3, distance: 5}, "fast");
            $('select#frequency[value=""]').effect('shake', {times: 3, distance: 5}, "fast");
            return false;
        }
    ...
    <div class="content_main">
        <div class="compliance_box_top" style="min-height: 500px;">
            <div id="compliance_snapshot">
                <?php echo form_open("youaudit/franchise_admins/complianceChecksForFranchise/$masterid", array('id' => 'add_compliance', 'action' => 'post')); ?>
                                ...
                                <?php foreach ($allTests as $test) { ?>
                                    <tr>
                                        ...
                                        <td class="right action">
                                            <a  href="#EditTaskModal" data-toggle="modal" onclick="editTaskSetup(this)" data-id="<?php echo $test['id']; ?>"data-href="<?php echo base_url() . 'youaudit/franchise_admins/editTaskAdmins/' . $test['id'].'/'.$masterid; ?>"><img src="<?php echo 'http://' . $_SERVER['HTTP_HOST']; ?>/img/icons/16/modify.png" title="Edit" alt="Edit"></a>
                                            <a  href="javascript:void(0)" data-toggle="modal" onclick="deleteTask(this)" data-id="<?php echo $test['id']; ?>"data-href="<?php echo base_url() . 'youaudit/franchise_admins/removeTaskAdmins/' . $test['id'].'/'.$masterid; ?>"><img src="<?php echo 'http://' . $_SERVER['HTTP_HOST']; ?>/img/icons/16/erase.png" title="Remove" alt="Remove"></a>
                                        </td>
                                    </tr>
<?php } ?>
                            </thead>
...
<?php echo form_open("youaudit/franchise_admins/addTaskForFranchise/$masterid", array('id' => 'add_new_task')); ?>
