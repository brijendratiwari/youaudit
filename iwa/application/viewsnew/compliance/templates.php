<?php // print "<pre>"; print_r($allTemplates); print "</pre>"; ?>

<link href="<?php echo 'http://' . $_SERVER['HTTP_HOST']; ?>/youaudit/includes/css/sub_style.css" rel="stylesheet" type="text/css" />
<!--<link href="<?php echo 'http://' . $_SERVER['HTTP_HOST']; ?>/brochure/js/bootstrap/css/bootstrap.min.css" rel="stylesheet" type="text/css" />
<script type="text/javascript" language="javascript" src="<?php echo 'http://' . $_SERVER['HTTP_HOST']; ?>/brochure/js/bootstrap/js/bootstrap.min.js"></script>-->
<style>
    .DTTT_container{
        display: block;
    }
...
<div class="box_content">
    <div class="ver_tabs">
      ...
      <a class="active" href="#"><span>Templates</span></a>
    ...
    <div class="content_main">
        ...
        <div id="list" class="form_block">
            ...
            <table id="compliance_list" class="list_table">
                <thead>
                    <tr><?php if(isset($this->session->userdata('objSystemUser')->levelid)){
                ...
                <tbody>
                    <?php foreach ( $allTemplates as $test) { ?>
                    <tr data-value="<?php print $test['cid']; ?>">
                        <?php if(isset($this->session->userdata('objSystemUser')->levelid)){
                        if($this->session->userdata('objSystemUser')->levelid == 4){
                        ?>
                      ...
                      <td hidden=""><?php print $test['tasks']; ?></td>
                       <td class="">
                            ...
                            <a href="#complianceAddModal" class="add_b" data-toggle="modal"><img src="/img/add.jpg" title="Add to List" alt="Add to List"></a>
                            <a href="#complianceEditModal" class="edit_b" data-toggle="modal"><img src="/img/icons/16/modify.png" title="Edit Check" alt="Edit Check"></a>
    ...
    <div class="modal fade" id="complianceEditModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <form action="<?php echo base_url()?>compliance/addModifiedTemplate" method="post">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                        <h4 class="modal-title" id="myModalLabel">Edit and Add Check to Compliance List</h4>
                    </div>
...
<!--                        <div class="row">
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="submit" onclick="return beforeTestEdit('#complianceEditModal')" class="btn btn-warning">Add to My List</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <div class="modal fade" id="complianceAddModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <form action="<?php echo base_url()?>compliance/addModifiedTemplate" method="post">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                        <h4 class="modal-title" id="myModalLabel">Add Check to Compliance List</h4>
                    </div>
                    <div class="modal-body">
                        <div class="row">
                            <input name="compliance_check_id" hidden="">
...
<!--                        <div class="row">
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                        <button type="submit" onclick="return beforeTestEdit('#complianceAddModal')" class="btn btn-warning">Add to My List</button>
                    </div>
                </form>
            </div>
        </div>
...
<div class="modal fade" id="multiComplianceEditModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <form action="<?php echo base_url()?>compliance/editMultiTemplates" method="post">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
    ...
    <script>
        ...
        var options = '';
            ...
            <?php }?>
            var rowcount = 0;    
            <?php if($this->session->userdata('objSystemUser')->levelid == 4){ ?>
                rowcount = 1;
            <?php }?>
        $(document).ready(function() {
                var colCount =0;
...
//------------Select All check----------
            ...
            $('body').on('click','.edit_b',function(){
                $('#complianceEditModal').find('form').trigger('reset');
                var row = $(this).parent('td').parent('tr');
                var rowData = table.row( row ).data();
                console.log(rowData);
                var name = rowData[rowcount + 0];
                var id = rowData[rowcount + 4];
                var frequency = rowData[rowcount + 1];
                var mandatory = rowData[rowcount + 3];
                var tasks = rowData[rowcount + 5];
                ...
                $('#complianceEditModal').find('#newTasks_table tbody').html('');
                $('#complianceEditModal').find('button[type="submit"]').prop('disabled',true);
                $('#complianceEditModal').find('#newTasks_table tbody').html('<tr><td colspan="3"><img width="15" src="<?php echo base_url('../img/ajax-loader.gif'); ?>"><span>&nbsp;&nbsp;Please Wait...</span></td></tr>');
                $.ajax({
...
//                        alert(data);
                        ...
                        $('#complianceEditModal').find('#newTasks_table tbody').html(tasks);
                        var i = 0;
                        $.each(data,function(k,v){
                           $('#complianceEditModal').find('#newTasks_table').find('.task_row').eq(i).find('.check_type option[value="'+v['type_of_task']+'"]').prop('selected',true);
                           $('#complianceEditModal').find('#newTasks_table').find('.task_row').eq(i).find('.add_measurements option[value="'+v['measurement']+'"]').prop('selected',true);
                           if(v['measurement'] > 0){
                                $('#complianceEditModal').find('#newTasks_table').find('.task_row').eq(i).find('.add_measurements').prop('disabled',false);
                            }
                           i++;
                        });
                    },
            ...
            $('body').on('click','.add_b',function(){
                $('#complianceAddModal').find('form').trigger('reset');
                var row = $(this).parent('td').parent('tr');
                var rowData = table.row( row ).data();
//                console.log(rowData);
                var name = rowData[rowcount + 0];
                ...
                $('select[name="mandatory"] option[data-selector="'+mandatory+'"]').prop('selected',true);
                $('select[name="frequency"] option[data-selector="'+frequency+'"]').prop('selected',true);
                $('#complianceAddModal').find('#newTasks_table tbody').html('');
                $('#complianceAddModal').find('button[type="submit"]').prop('disabled',true);
...
//                        alert(data);
                        var tasks = '';
                        $('#complianceAddModal').find('button[type="submit"]').prop('disabled',false);
                        $.each(data,function(k,v){
                            
                            tasks += '<tr class="task_row" data-new="false"><td><input readonly="" class="task_name_input" placeholder="Task Name"  data-id="'+v['id']+'" type="text" value="'+v['task_name']+'" /></td><td>Type: <select class="check_type" disabled="" ><option value="0">Standard</option><option value="1">Numerical</option></select></td> <td><select disabled="true" class="add_measurements"><option value="0">Select Measurement</option>'+options+'</select></td><!--td><img src="/img/icons/16/erase.png" data-task="'+v['id']+'" class="remove_old_task" title="Remove" alt="Remove"></td--></tr>';
                        });
...
//----------------------modal Task Remove Code------------------
    ...
    function beforeTestEdit(modalId){
        var tasksArr = [],tasksStr;
        $.each($(modalId).find('.task_row'),function(){
        ...
        $(modalId).find('#task_details').val(tasksStr);
    }
