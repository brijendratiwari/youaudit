...
//            $this->db->join('items_pat_link', 'items.pattest_status = items_pat_link.pattest_status', 'left');
            ...
            foreach ($arrFilters as $strField => $strValue) {
                $arrFieldInfo = explode("_", $strField);

                if ($arrFieldInfo[1] == "start") {
                    $this->db->like($this->getTableAndFieldForFilterField($arrFieldInfo[0]), $strValue, 'both');
                }
                if ($arrFieldInfo[1] == "exact") {
                    $this->db->where($this->getTableAndFieldForFilterField($arrFieldInfo[0]), $strValue);
                }
            }

            if (count($arrOrder) > 0) {
                $this->db->order_by($this->getTableAndFieldForFilterField($arrOrder[0]), $arrOrder[1]);
            }

            if (!$booGetCount) {
                if ((!array_key_exists('limit', $arrPagination)) || (array_key_exists('limit', $arrPagination) && ($arrPagination['limit'] != 'none'))) {
                    if ($export == '') {

                        if ((count($arrPagination) == 0)) {
                            $this->db->limit(10, 0);
                        } else {
                            if (array_key_exists('page', $arrPagination)) {
                                $this->db->limit($arrPagination['limit'], $arrPagination['limit'] * ($arrPagination['page'] - 1));
                            } else {
                                $this->db->limit($arrPagination['limit'], 0);
                            }
                        }
                    }
                }
            ...
            if ($export == 'CSV') {
                ...
                force_download(date('d/m/Y Gis') . '.csv', $this->dbutil->csv_from_result($resQuery));
            } elseif ($export == 'PDF') {
                $this->load->model('users_model');

                $selected_field = $this->users_model->getColumns($this->session->userdata('objSystemUser')->userid);

                foreach ($selected_field as $selected_column) {
                    if ($selected_column[0]->input_name == 'manu_model') {

                        $myFields[] = array('strName' => 'Manufacturer', 'strFieldReference' => 'manufacturer', 'arrFooter' => array('booTotal' => false, 'booTotalLabel' => false, 'intColSpan' => 0));
                        $myFields[] = array('strName' => 'Model', 'strFieldReference' => 'model', 'arrFooter' => array('booTotal' => false, 'booTotalLabel' => false, 'intColSpan' => 0));
                    } elseif ($selected_column[0]->input_name == 'owner') {
                        $myFields[] = array('strName' => 'First Name', 'strFieldReference' => 'userfirstname', 'arrFooter' => array('booTotal' => false, 'booTotalLabel' => false, 'intColSpan' => 0));
                        $myFields[] = array('strName' => 'Last Name', 'strFieldReference' => 'userfirstname', 'arrFooter' => array('booTotal' => false, 'booTotalLabel' => false, 'intColSpan' => 0));
                    } else {
                        $myFields[] = array('strName' => $selected_column[0]->name, 'strFieldReference' => $selected_column[0]->input_name, 'arrFooter' => array('booTotal' => false, 'booTotalLabel' => false, 'intColSpan' => 0));
                    }
                }
                $arrFields = array(
                    array('strName' => 'Barcode', 'strFieldReference' => 'barcode', 'arrFooter' => array('booTotal' => false, 'booTotalLabel' => false, 'intColSpan' => 0)),
                    array('strName' => 'Serial No', 'strFieldReference' => 'serial_no', 'arrFooter' => array('booTotal' => false, 'booTotalLabel' => false, 'intColSpan' => 0)),
                    ...
                    array('strName' => 'First Name', 'strFieldReference' => 'userfirstname', 'arrFooter' => array('booTotal' => false, 'booTotalLabel' => false, 'intColSpan' => 0)),
                    array('strName' => 'Last Name', 'strFieldReference' => 'userfirstname', 'arrFooter' => array('booTotal' => false, 'booTotalLabel' => false, 'intColSpan' => 0)),
                    array('strName' => 'Nick Name', 'strFieldReference' => 'usernickname', 'arrFooter' => array('booTotal' => false, 'booTotalLabel' => false, 'intColSpan' => 0)),
...
//                var_dump($arrFields);
//die;
                $this->outputPdfFile(date('d/m/Y Gis') . '.pdf', $myFields, $resQuery->result_array());
            }
...
//              die(); 
            ...
            if (!$booGetCount) {
                if ($resQuery->num_rows() > 0) {
                    if ($resQuery->num_rows() == 1) {
                        $item = $resQuery->row();
                        redirect('items/view/' . $item->itemid);
                    }
                    $arrItemsData = array();
                    foreach ($resQuery->result() as $objRow) {
                        $arrItemsData[] = $objRow;
    ...
    public function linkThisToLocation($intItemId = -1, $intLocationId = -1) {
        if (($intItemId > 0) && ($intLocationId > 0)) {
            $this->db->insert('items_locations_link', array('item_id' => $intItemId, 'location_id' => $intLocationId, 'date' => date('Y-m-d H:i:s')));
...
//             var_dump($resQuery->row()->locationid);die;
    ...
    public function getRecentlyDeleted($intAccountId) {
        if ($intAccountId > 0) {
            $this->db->select('items.id, items.barcode,items.item_manu, items.manufacturer, items.model, items.mark_deleted_date,
...
WHERE item_log.item_id = '" . $intID . "' AND item_log.account_id = '" . $this->session->userdata('objSystemUser')->accountid . "' ORDER BY item_log.id DESC");
    ...
    public function update_file($itemid, $filename) {
        ...
        if ($check != '') {
            $this->db->query("UPDATE items SET pdf_name =CONCAT(pdf_name ,'," . $filename . "') WHERE id =" . (int) $itemid);
        } else {
            $this->db->query("UPDATE items SET pdf_name = '" . $filename . "' WHERE id =" . (int) $itemid);
        }
    }
...
// Delete pdf file from database which is shown on apple devices
    public function delete_file($id, $filename) {
        ...
        $pdf_name = explode(',', $query->row()->pdf_name);
        $doc_arr = array();
...
//        echo $doc_string;
        ...
        if ($doc_string) {
            $this->db->query("UPDATE items SET pdf_name ='" . $doc_string . "' WHERE id =" . (int) $id);
        } else {
            $this->db->query("UPDATE items SET pdf_name ='" . NULL . "' WHERE id =" . (int) $id);
        }
        return TRUE;
    }

    public function updateOne($item_id, $intPhotoId) {
        if ($intPhotoId > 0) {
...
//          Add Quanitity Fields
            ...
            $arrItemData = array(
                ...
                'owner_now' => $this->input->post('user_id'),
...
//        return true;
//    }
    public function recordCheck($data, $itemID) {
//        return 'hello';die;
        $set = array();
        $other = array();
        $failed = array();
    ...
    public function updateMultipleItems($data) {
        ...
        if ($data['items_id'] != '') {
            ...
            if ($data['item_warranty']) {
                $warranty_date = date('Y-m-d', strtotime($data['item_warranty']));
            }
...
//            var_dump($arrItemsData1);
	...
	public function getAllFaultItems($intAccountId = -1) {
        if ($intAccountId > 0) {
		...
		locations.id AS locationid, locations.name AS locationname,
                sites.id AS siteid, sites.name AS sitename,
                pat.pattest_name AS pat_status,
                itemstatus.name AS statusname,tickets.description,
                suppliers.supplier_title AS suppliers_title,tickets.severity,items.account_id,tickets.id as ticket_id,tickets.fix_code,tickets.reason_code,tickets.order_no,tickets.jobnote,tickets.date as dt');
            ...
            $this->db->join('locations', 'items.location_now = locations.id', 'left');
            $this->db->join('sites', 'items.site = sites.id', 'left');
            $this->db->join('tickets', 'tickets.item_id = items.id', 'right');
			$this->db->join('suppliers', 'items.supplier = suppliers.supplier_id', 'left');
            $this->db->join('itemstatus', 'items.status_id = itemstatus.id', 'left');
			...
			$this->db->where('items.account_id', $intAccountId);
			$this->db->where('items.status_id =', 2);
			$this->db->or_where('items.status_id =', 3); 
            $this->db->where('items.active', 1);
	...
	public function basicGetOneWithTicket($intItemId = -1, $intAccountId = -1) {
        if (($intItemId > 0) && ($intAccountId > 0)) {
            $this->db->select('
                        items.id AS itemid,
                        items.manufacturer,
                        ...
                        sites.name AS sitename,
                        items.supplier,
                        suppliers.supplier_title AS suppliers_title,tickets.id as ticket_id,tickets.fix_code,tickets.reason_code,tickets.order_no,tickets.jobnote,,tickets.ticket_action,tickets.date as dt,tickets.order_no');
            $this->db->from('items');
            $this->db->join('users', 'items.owner_now = users.id', 'left');
            $this->db->join('photos', 'photos.id = items.photo_id', 'left');
            $this->db->join('locations', 'items.location_now = locations.id', 'left');
