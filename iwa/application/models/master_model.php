...
class Master_Model extends CI_Model {
    ...
    public function getSysAdminName($master_account_id) {
        $this->db->select('sys_admin_name');
        $this->db->where('id', $master_account_id);
        $res = $this->db->get('master_ac');
        $result = $res->result_array();
        if ($result) {
            return $result;
    ...
    public function addAdminUser($arrAdminUser) {

        if (isset($arrAdminUser)) {


            $this->db->insert('systemadmin_master', $arrAdminUser);
            return 1;
        } else {
            return FALSE;
        }
    ...
    public function editAdminUser($editAdminUser) {
    ...
    public function changeAdminUserPassword($changeAdminUserPassword) {
    ...
    public function addCustomerAc($arrCustomer) {
        ...
        // check account limit of master account
    ...
    public function editCustomerAc($editArrCustomer) {
        if (isset($editArrCustomer)) {
            ...
            $persnal_info = array(
                'firstname' => $editArrCustomer['firstname'],
                'lastname' => $editArrCustomer['lastname'],
                'nickname' => $editArrCustomer['contact_name']
            );
            if ($this->input->post('edit_contact_password')) {
                $persnal_info['password'] = $this->input->post('edit_contact_password');
            }
    ...
    // check Reference code is present already or not in DATABASE
    public function checkRefcode($reference_code) {
    ...
    public function check_masterusername($username) {
    ...
    public function geteditCustomerdata($customer_id) {
        if (isset($customer_id)) {
    ...
    public function updateUserPassword($changeUserPassword) {
    ...
    //Action For Edit Master Customer User.
    public function editUser($editCustomerUser) {
    ...
    public function summary($master_id) {

        $this->db->where(array('account_id' => $master_id, 'account_type' => 1));
        $rs = $this->db->get('accounts')->result();
        $total = count($rs);

        $where = array('account_id' => $master_id, 'active' => 1, 'account_type' => 1);
        $this->db->where($where);
    ...
    public function getrecentaccounts($master_id) {

        $this->db->select('accounts.name AS company_name,accounts.package_id,packages.name,packages.item_limit');
        $this->db->where(array('accounts.account_id' => $master_id, 'accounts.account_type' => 1));
        $this->db->from('accounts');
        $this->db->join('packages', 'accounts.package_id=packages.id');
    ...
    public function addProfile($owner, $category, $manu, $manufacturer) {
        $data = $this->session->userdata('AdminUserMaster');
        ...
        $profiledata = array(
            ...
            'account_id' => $data['master_account_id'],
            'account_type' => 1
        );


        $rs = $this->db->insert('profile', $profiledata);
    ...
    public function profilelist() {
        $data = $this->session->userdata('AdminUserMaster');
        $this->db->where('account_id', $data['master_account_id']);
        $this->db->where('account_type', 1);
        ...
        foreach ($arr as $key => $value) {
            ...
            if (!empty($newArr)) {
                if ($newArr[0] == 'true') {
                    ...
                    if ($newArr[3] == 1)
                        $temp = array('task_name' => $newArr[1], 'type_of_task' => $newArr[3], 'Measurement' => $newArr[4], 'template_task' => 1, 'admin_id' => $data['masterid'], 'account_type' => 1);
                    else
                        $temp = array('task_name' => $newArr[1], 'type_of_task' => $newArr[3], 'Measurement' => '', 'template_task' => 1, 'admin_id' => $data['masterid'], 'account_type' => 1);
...
//        }
    ...
    public function getAllTasksForMasterAdmins($masterid) {
        $sql = "SELECT tasks.id, tasks.task_name, tasks.type_of_task, measurements.measurement_name, measurements.id as mid  FROM tasks LEFT JOIN measurements ON tasks.measurement = measurements.id WHERE tasks.account_id = '0' and tasks.template_task = '1' and tasks.archive = '0' and admin_id=$masterid and account_type='1'";
    ...
    public function getAllMasterCompliances($masterid) {
        ...
        $this->db->where('compliance_template.account_type', 1);
        ...
        foreach ($arr as $key => $value) {
            ...
            if (!empty($newArr)) {
                if ($newArr[0] == 'true') {
                    ...
                    if ($newArr[3] == 1)
                        $temp = array('task_name' => $newArr[1], 'type_of_task' => $newArr[3], 'Measurement' => $newArr[4], 'account_id' => 0, 'template_task' => 1, 'admin_id' => $masterid, 'account_type' => 1);
                    else
                        $temp = array('task_name' => $newArr[1], 'type_of_task' => $newArr[3], 'Measurement' => '', 'account_id' => 0, 'template_task' => 1, 'admin_id' => $masterid, 'account_type' => 1);
                    $this->db->insert('tasks', $temp);
                    $id = $this->db->insert_id();
                    $idArr[] = $id;
                }
...
//        $dateOutput = $dateInput[2] . '-' . $dateInput[1] . '-' . $dateInput[0];
    ...
    public function getProfile($profile_id) {
        if ($profile_id) {
            ...
            foreach ($result as $record) {
                $custom = json_decode($record['custom_field']);
                if ($custom) {
                    for ($i = 0; $i < count($custom->name); $i++) {
                        $field_names[] = $custom->name[$i];
                    }
                    for ($i = 0; $i < count($custom->type); $i++) {
                        $field_types[] = $custom->type[$i];
                    }
                } else {
                    $field_names = 0;
                    $field_types = 0;
                ...
                $mydata = array(
                    ...
                    'cus_name' => $field_names,
                    'cus_type' => $field_types
                );
            }
    ...
    public function expotMasterCusPdf($export, $account_id) {
        if ($export != '') {
...
COUNT( accounts.id ) >=1
    ...
    public function exportAdminUser($export, $master_id) {
        if ($export != '') {
            $query = "select firstname,lastname,username
                  FROM systemadmin_master WHERE master_account_id =" . $master_id;
        }
        $res = $this->db->query($query);
    ...
    public function exportProfilePdf($export, $masterid) {
        if ($export != '') {
            $query = "select profile_name,owner,category,manu,manufacturer
                  FROM profile WHERE account_type = '1' AND account_id =" . $masterid;
        }
        $res = $this->db->query($query);
...
// check unique qrcode
    ...
    public function editMultiple_MasterAc() {
        ...
        if ($data['account_id'] != '') {
            ...
            if (empty($update_list)) {
                ...
                $this->db->where_in('id', $ids)->update('master_ac', $update_list);
            }
            return TRUE;
        }
    }
